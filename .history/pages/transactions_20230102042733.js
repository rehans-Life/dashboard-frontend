...
export default function Transactions() {
  ...
  // We dont want to make an API call for every letter that
  // the user types within the search field we will only do it
  // when the user clicks the search button

  // Hence we will create a temporary state that is going to get
  // updated upon each modification of the search field but only
  // when the user clicks on the search icon am i going to
  // transfer the other states value to this state and hence that
  // search input will go to the backend and it will return those
  // documents whose cost or userId field matches the search input.
  ...
  // Its only going to make another API call only when the mentioned
    ...
    <Box m="1.5rem 2.5rem">
      ...
      >
        ...
        <DataGrid
          ...
          page={pageNo}
          rowCount={data?.total}
          // Providing the option to the user to change the page
          // size when that happens the onPageSizeChange function
          // will run and the API call will be made as the state
          // will be set and we will grab the data accordingly
          ...
          // Sending the searchInput state, setSearchInput func and
          // the setSearch func into our custom toolbar component
          componentsProps={{
