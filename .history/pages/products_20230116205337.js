  ...
  return (
    ...
    >
      <CardContent>
        ...
        <Box marginY="1rem">
          <Rating readOnly name="size-medium" value={Math.floor(rating)} />
          <Typography variant="body2">{description}</Typography>
        </Box>
        <Button onClick={handleExpand} size="small" variant="primary">
          SEE MORE
        </Button>
        <Collapse
          in={isExpanded}
        ...
        >
          <CardContent>
            {Object.entries(collapseInfo).map((stat) => (
              <Typography key={stat[0]} sx={{ overflowX: "hidden" }}>
                {stat[0]} : {stat[1]}
              </Typography>
            ))}
          </CardContent>
