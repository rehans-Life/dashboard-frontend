...
export const api = createApi({
  ...
  endpoints: (build) => ({
    ...
    // Creating a query method which is going to make a request
    // to the management/performance/:id route to get a user
    // and there affiliate purchases information.

    // Now whenever this query method is triggered i will be
    // required to pass in the id of the user whose information
    // i want from the database.

    // Then that id is sent in the url to the backend where the
    // the query is made to grab that users information along
    // with its affiliate purchases information.

    getUserPerformace: build.query({
      query: (id) => `management/performance/${id}`,
      providesTags: ["Performance"],
    }),
    gerDashboardInformation: build.query({
...
export const {
  ...
  useGerDashboardInformationQuery
  useGetSalesQuery,
  useGetAdminsQuery,
